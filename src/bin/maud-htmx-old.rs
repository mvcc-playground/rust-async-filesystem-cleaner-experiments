    ...
    html! {
        ...
        html {
            head {
                ...
                // link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/@unocss/runtime";
                link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/htmx.org@1.9.10";

                // Recomendado para desenvolvimento rápido (faz as classes funcionarem sem build)
    ...
    layout(
        ...
        html! {
            div class="flex flex-col gap-4 items-center justify-center h-screen" {
                ...
                button id=(id) class="bg-red-500 text-white rounded-md px-3 py-1" onclick="alert('Hello, World!')" { "Click me" }
            ...
            (PreEscaped(format!("<style> button {{ @apply bg-blue-500 text-white rounded-md px-4 py-2 }} </style>")))
        },
    )
}
